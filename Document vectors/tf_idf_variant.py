...
def main():
    ...
    def tf_max_document(document):
        """Calculate max term frequency in document."""
        corpus = list(set(document))
        frequency = [0] * len(corpus)
        for term in document:
            frequency[corpus.index(term)] += 1
        ...
        [
            [
                (
                    ...
                    + (1 - a)
                    * document.count(term)
                    / tf_max_document(document=document)
                )
                * np.log10(
                    len(documents)
                    / len([document for document in documents if term in document])
                )
                for term in terms
            ]
